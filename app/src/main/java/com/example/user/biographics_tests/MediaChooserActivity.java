...
//                }
                ...
                try {
                    ...
                    //TODO Add the media title to the db.
                    ...
                    timeLine.putExtra("2ndTitle", scndTitle);
                    ...
                    storyDetails.putExtra("type", "Photo");
                    storyDetails.putExtra("tPath", tumb);
                    storyDetails.putExtra("2ndTitle", scndTitle);
                    startActivity(storyDetails);
                    return;
                }
            }
...
//        else if(!idOrNewOrCampaignQ.equals("new"))
        try {
            ...
            Intent timeLine = new Intent(getApplicationContext(), DorActivity.class);
            timeLine.putExtra("uid", id);
            timeLine.putExtra("type", "Video");
            timeLine.putExtra("2ndTitle", scndTitle);
            startActivity(timeLine);
            return;
        } catch (NumberFormatException e) {
            //if caught then it is not new not number, it is a question from campaign or new.
            Intent storyDetails = new Intent(getApplicationContext(), StoryDetails.class);
            storyDetails.putExtra("path", videoPath);
            storyDetails.putExtra("tPath", tumb);
            storyDetails.putExtra("type", "Video");
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        ...
        if (resultCode == RESULT_OK &&
            ...
            popUpTitle = "Video";
            FolderId++;
        } else if (requestCode == RECORD_REQUEST && data != null) {
//            if (data == null) {
...
//                } else if (!idOrNewOrCampaignQ.equals("new")) {
            ...
            try {
                ...
                db.close();
                next = new Intent(this, DorActivity.class);
                next.putExtra("uid", id);
               // startActivity(next);
               // return;
            } catch (NumberFormatException e) {
                //if caught then it is not new not number, it is a question from campaign or a new story
                next = new Intent(this, StoryDetails.class);
                next.putExtra("path", path);
                next.putExtra("idOrNewOrCampaignQ", idOrNewOrCampaignQ);
               // startActivity(next);
               // return;
            }
        } else {
        ...
        MaterialDialog dialog = new MaterialDialog.Builder(MediaChooserActivity.this)
                ...
                .onPositive(new MaterialDialog.SingleButtonCallback() {
                    ...
                    public void onClick(MaterialDialog materialDialog, DialogAction dialogAction) {
                        ...
                        } else if (popUpTitle == "Video") {
                            videoChooserManager.submit(requestCode, data);
                        } else {
                            next.putExtra("type", "Audio");
                            next.putExtra("2ndTitle",scndTitle);
                            startActivity(next);
                        }
                    }
                })
